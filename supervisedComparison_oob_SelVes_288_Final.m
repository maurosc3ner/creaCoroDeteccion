...
tabulate(yTrain)
tabulate(yTest(:,4))
load models/rbVSrf_250_tm0_oob_SelVes_288
...
toc
tab = tabulate(yTest(:,4));
[cm order]=confusionmat(yTest(:,4),str2num(cell2mat(Yfit3)));
cm2=bsxfun(@rdivide,cm,tab(:,2))*100;
...
[fpr1,tpr1] = perfcurve(yTest(:,4),adbscore(:,1),'1');
plot(fpr1,tpr1,'r-');
[fpr2,tpr2] = perfcurve(yTest(:,4),rtscore(:,1),'1');
hold on;
plot(fpr2,tpr2,'k-.');
[fpr3,tpr3] = perfcurve(yTest(:,4),rfscore(:,1),'1');
hold on;
plot(fpr3,tpr3,'b--');
title('ROC curve without distance');
